...
from assets.settings import WHITE, BLACK, RED, CYAN, MAGENTA, BLUE, GREEN, YELLOW, GRAY
from assets.settings import MOVE_LEFT, MOVE_RIGHT, MOVE_DOWN, MOVE_UP
from assets.settings import WIDTH_UNIT, WINDOW_WIDTH, WINDOW_HEIGHT
from assets.circuit_node_types import (EMPTY, IDEN, X, Y ,Z, S, SDG, T, TDG, H, SWAP, CTRL, TRACE)
from assets.circuit_grid_model import CircuitGridNode
from assets.resources import load_image

# Global Vars
GRID_WIDTH = WIDTH_UNIT * 4.96 # Multiplied by 20
GRID_HEIGHT = GRID_WIDTH # Multiplied by 3
...
class CircuitGrid(pygame.sprite.RenderPlain):
    ...
    def __init__(self, circuit_grid_model, game):
        self.xpos = 20 + 80#(WINDOW_WIDTH - GRID_WIDTH * (circuit_grid_model.max_columns + 2))/2 # 20 # Standard Radius of Window Corner Curve on MacOS
        self.ypos = WINDOW_HEIGHT - 20 - GRID_HEIGHT*(circuit_grid_model.max_wires+1)
        self.circuit_grid_model = circuit_grid_model
        self.game = game
        self.enabled = True
        self.selected_wire = 0
        self.selected_column = 0
        self.circuit_grid_background = CircuitGridBackground(circuit_grid_model, self.xpos)
        self.circuit_grid_cursor = CircuitGridCursor()
        self.gate_tiles = np.empty((circuit_grid_model.max_wires, circuit_grid_model.max_columns), dtype=CircuitGridGate,)

        for row_idx in range(self.circuit_grid_model.max_wires):
            for col_idx in range(self.circuit_grid_model.max_columns):
                self.gate_tiles[row_idx][col_idx] = CircuitGridGate(circuit_grid_model, row_idx, col_idx)

        pygame.sprite.RenderPlain.__init__(self, self.circuit_grid_background, self.gate_tiles, self.circuit_grid_cursor,)
        self.update()
    ...
    def update(self):
        ...
        for row_idx in range(self.circuit_grid_model.max_wires):
            for col_idx in range(self.circuit_grid_model.max_columns):
                self.gate_tiles[row_idx][col_idx].rect.centerx = self.xpos + GRID_WIDTH * (col_idx + 1.5)
                self.gate_tiles[row_idx][col_idx].rect.centery = self.ypos + GRID_HEIGHT * (row_idx + 1.0)
    ...
    def handle_input(self):
        ...
        if actions["toggleCircuitGrid"] and self.enabled:
            if actions["left"]:
            ...
            #     self.handle_input_rotate(np.pi / 8)
        self.enabled = self.game.cooldowns["circuitGridInput"] == 0
    ...
    def handle_input_x(self):
        ...
        if selected_node_gate_part == EMPTY:
            if self.game.stateStack[-1].player.quantumSpellBook['X'] > 0:
                self.game.stateStack[-1].player.quantumSpellBook['X'] -= 1
                self.game.stateStack[-1].levelUI.updateCount('X', -1)
    ...
    def handle_input_h(self):
        ...
        selected_node_gate_part = self.get_selected_node_gate_part()
        if selected_node_gate_part == EMPTY:
            if self.game.stateStack[-1].player.quantumSpellBook['H'] > 0:
                self.game.stateStack[-1].player.quantumSpellBook['H'] -= 1
                self.game.stateStack[-1].levelUI.updateCount('H', -1)
...
class CircuitGridBackground(pygame.sprite.Sprite):
    ...
    def __init__(self, circuit_grid_model, xpos):
        pygame.sprite.Sprite.__init__(self)

        width = GRID_WIDTH * (circuit_grid_model.max_columns + 2) # (WINDOW_WIDTH - 2*xpos) 
        height = GRID_HEIGHT * (circuit_grid_model.max_wires + 1)
        self.image = pygame.Surface([width, height]) # GRID_WIDTH * (18 + 2)
        self.image.convert()
...
class CircuitGridCursor(pygame.sprite.Sprite):
    ...
    #     selected_node_gate_part = self.get_selected_node_gate_part()
    #     if selected_node_gate_part in (X, Y, Z, H):
    ...
    #     """
    #     selected_node_gate_part = self.get_selected_node_gate_part()
    #     if selected_node_gate_part in (X, Y, Z):
    #         circuit_grid_node = self.circuit_grid_model.get_node(
    #             self.selected_wire, self.selected_column
    #         )
    ...
    #         return -1
    #     candidate_wire_gate_part = self.circuit_grid_model.get_node_gate_part(candidate_ctrl_wire_num, self.selected_column)
    #     if candidate_wire_gate_part in (EMPTY, TRACE):
    ...
    #         self.circuit_grid_model.set_node(candidate_ctrl_wire_num, self.selected_column, CircuitGridNode(EMPTY),)
    #         self.update()
